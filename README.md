...
- 多账号 Basic Auth 认证，账号数据按 `accounts/<uuid>/` 前缀隔离。
...
`wrangler.jsonc` 已配置每 15 分钟 Cron。只有存在待处理移除任务时才扫描管理 KV；每次最多处理一个、最多删除 500 个对象的账号移除批次。已完成或失败任务的摘要在 30 天后清理。`ADMIN_CSRF` 同时会在账号移除开始后拒绝新的 WebDAV 写请求，并等待已开始的写请求结束，避免遗留无账号记录可恢复的加密对象。
...
- 查看账号状态及账号移除任务状态，并显示管理员会话剩余时长。
- 启用或停用账号。
- 重设密码。此操作仅替换密码盐和哈希，不改变数据密钥、账号 ID 或既有备份对象。
- 只读浏览某个账号的备份元数据：目录层级、路径前缀筛选、逐级返回的面包屑导航、每页最多 100 项，以及路径、类型、易读格式的明文大小、创建时间、修改时间和 ETag。
- 发起、暂停或继续账号移除任务；失败任务会显示失败对象路径与错误摘要，排查后可重新发起移除。
- 操作失败时在概览页显示具体原因，创建成功使用独立的成功提示。

管理面不会显示、写入日志或在提交后重新展示 WebDAV 密码；也不能下载、解密、预览、编辑或删除单个备份对象。

账号记录必须包含 `storagePrefix: "accounts/<uuid>/"`。从旧版升级时，请先移除旧账号的 `u/<uuid>/` R2 对象与 `users/<账号名>` KV 记录，再通过管理面重新创建账号并上传备份；本项目不兼容旧 `u/<uuid>/` 布局。
